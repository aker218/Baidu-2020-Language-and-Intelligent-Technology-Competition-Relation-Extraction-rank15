...
class RelDataset(Data.Dataset):
    ...
    def __getitem__(self,idx):
        return self.input_ids[idx],self.attention_mask[idx],self.token_type_ids[idx],               self.rel_label[idx],self.postag[idx],self.feature[idx],self.token2docs[idx]  
...
if not os.path.exists("./middle_data/cls_examples.pk"):
    ...
    for idx,(tokens,rel,token2doc,postag) in tqdm(enumerate(zip(cls_doc_tokens,cls_doc_rels,cls_token2doc,text_data_postag))):
                cls_examples.append(create_cls_example(tokens,rel,token2doc,postag,texts_tfidf_svd[idx],tokenizer))   
...
cls_train_dataset=RelDataset(cls_examples[:180000])
cls_val_dataset=RelDataset(cls_examples[180000:])
...
#          adam_epsilon=adam_epsilon,num_train_epochs=epochs,warmup_steps=0,gradient_accumulation_steps=1,save_steps=11250,
#          max_grad_norm=1.0,model_name_or_path=output_dir,output_dir=output_dir,seed=42,device=device,n_gpu=0,
...
args=ARG(train_batch_size=batch_size*2,eval_batch_size=batch_size*2,weight_decay=weight_decay,learning_rate=learning_rate,
         adam_epsilon=adam_epsilon,num_train_epochs=epochs,warmup_steps=0,gradient_accumulation_steps=1,save_steps=5625,
...
#             model = torch.nn.DataParallel(model)
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            inputs = {"input_ids": batch[0], "attention_mask": batch[1], "rel_label": batch[3],                     "postag":batch[4],"feature":batch[5]}
#             inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch[3]}
            inputs["token_type_ids"]=batch[2]
#                 if args.model_type != "distilbert":
#                     inputs["token_type_ids"] = (
...
model=BertMulticlass_DGCNN(bert_dir,config_file,num_rel_labels=num_rel_labels,
                  hidden_dropout_prob=hidden_dropout_prob,embed_dim=128,loss_type=loss_type,use_feature=use_feature)
